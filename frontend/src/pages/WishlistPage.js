...
const WishlistPage = () => {
        ...
        if (item.image_url) {
          try {
            const res = await axios.get(
              `${process.env.REACT_APP_API_URL || "http://56.228.36.23"}/api/image-base64/${item.image_url}`
            );
  ...
  const handleMoveToCart = (product) => {
    ...
    toast.success("✅ Moved to Cart", { position: "top-center" });
    navigate("/cart");
  };
  ...
  const handleClearWishlist = () => {
    clearWishlist();
    toast.warning("🧹 Wishlist Cleared", { position: "top-center" });
  };
